...
(async () => {
	...
	const ui = (() => {
		...
		ui.graph = (() => {
			...
			graph.container = document.createElement('div');
			graph.container.style.cssText = 'position: fixed; bottom: 200px; right: 0; width: 200px; height: 100px; \
				z-index: 2; user-select: none;';
			graph.container.style.display = 'none';

			graph.canvas = document.createElement('canvas');
			graph.canvas.style.cssText = 'position: absolute; bottom: 0; right: 0; width: 200px; height: 50px;';
			graph.canvas.height = 50;
			graph.canvas.width = 200;
			graph.container.appendChild(graph.canvas);

			/** @type {[number, number] | undefined} */
			graph.mouse = undefined;
			graph.canvas.addEventListener('mousemove', e => {
				graph.mouse = [e.offsetX * devicePixelRatio / graph.canvas.width, e.offsetY * devicePixelRatio / graph.canvas.height];
				console.log('mouse move', graph.mouse);
			});
			graph.canvas.addEventListener('mouseleave', () => void (graph.mouse = undefined));

			graph.ctx = aux.require(
				graph.canvas.getContext('2d'),
				'Unable to get 2D context for the sigfixes graph. This is probably your browser being dumb, maybe ' +
				'reload the page?',
			);
			...
			/** @typedef {{
			 * 	worstPlace: number, bestPlace: number, score: number, timestamp: number,
			 * }} GraphFrame */
			/** @type {GraphFrame[]} */
			graph.frames = [];
			/** @type {GraphFrame} */
			graph.currentFrame = { worstPlace: 0, bestPlace: Infinity, score: 0, timestamp: Date.now() };

			setInterval(() => {
				graph.frames.push(graph.currentFrame);
				while (graph.frames.length > 86400_000 / 30_000) graph.frames.shift(); // clear frames after 24h
				graph.currentFrame = { worstPlace: 0, bestPlace: Infinity, score: 0, timestamp: Date.now() };
			}, 15_000); // TODO
	...
	const settings = (() => {
		...
		*/
		...
		addEventListener('mousedown', ev => {
			...
			if (!vanillaContainer.contains(target) && !sigmodContainer.contains(target)) return;
			for (const { container, help, helpbox } of containers) {
				if (container.contains(/** @type {Node | null} */ (target))) {
					if (target === help) helpbox.style.display = '';
				} else {
		...
		separator('• graph •');
		setting('Enable stats graph', [checkbox('graph')], () => true,
			'TODO stats graph');
		setting('Graph baseline', [slider('graphBaseline', 20000, 10000, 100000, 5000, 0)], () => settings.graph,
			'TODO stats graph');
			...
			ws.addEventListener('message', e => {
				...
				switch (dat.getUint8(0)) {
					case 0x10: { // world update
						...
						// (f) : update graph frame
						let globalScore = 0;
						for (const view of world.views.keys()) {
							globalScore += world.score(view);
						}
						ui.graph.currentFrame.score = Math.max(ui.graph.currentFrame.score, globalScore);
						break;
					}
					...
					case 0x31: { // ffa leaderboard list
						...
						vision.position = myPosition || Infinity; // if position is zero, then make it infinity

						let place = Infinity;
						for (const vision of world.views.values()) {
							place = Math.min(place, vision.position);
						}

						const { currentFrame } = ui.graph;
						currentFrame.worstPlace = Math.max(currentFrame.worstPlace, place);
						currentFrame.bestPlace = Math.min(currentFrame.bestPlace, place);
						break;
					}
	...
	const render = (() => {
		...
		/** @type {{
		 * 	frame: typeof ui['graph']['currentFrame'],
		 * 	dpr: number,
		 * 	visible: boolean,
		 * 	mouse: [number, number] | undefined,
		 * 	baseline: number 
		 * } | undefined} */
		let graphLastState = undefined;
		...
		function renderGame() {
			...
			(function statsGraph() {
				if (graphLastState && graphLastState.baseline === settings.graphBaseline
					&& graphLastState.dpr === devicePixelRatio && graphLastState.frame === ui.graph.currentFrame
					&& graphLastState.mouse === ui.graph.mouse && graphLastState.visible === settings.graph) return;

				if (!settings.graph && graphLastState?.visible) {
					ui.graph.container.style.display = 'none';
				} else if (settings.graph && !graphLastState?.visible) {
					ui.graph.container.style.display = '';
				}

				graphLastState = {
					baseline: settings.graphBaseline,
					dpr: devicePixelRatio,
					frame: ui.graph.currentFrame,
					mouse: ui.graph.mouse,
					visible: settings.graph,
				};

				if (!settings.graph) return;

				const maxFrames = 120; // TODO: don't hardcode this
				const { canvas, ctx, frames, mouse } = ui.graph;
				canvas.width = Math.ceil(200 * (devicePixelRatio - 0.0001));
				canvas.height = Math.ceil(200 * (devicePixelRatio - 0.0001));
				ctx.clearRect(0, 0, canvas.width, canvas.height);

				let maxScore = 0;
				for (let i = 0; i < maxFrames; ++i) {
					const frame = frames[frames.length - maxFrames + i];
					if (!frame) continue;
					if (frame.score > maxScore) maxScore = frame.score;
				}
				...
				// draw graph, but segment all parts where we were holding #1
				ctx.beginPath();
				ctx.moveTo(0, canvas.height);
				let lastFill;
				for (let i = 0; i < maxFrames; ++i) {
					const frame = frames[frames.length - maxFrames + i];
					const x = Math.floor(i / maxFrames * canvas.width);
					if (!frame) {
					...
					let fill;
					if (frame.bestPlace > 1) fill = '#09f'; // blue for normal
					else if (frame.worstPlace > 1) fill = '#fc0'; // yellow if was achieving first place
					else fill = '#0f0'; // bright green if completely solidified first place

					const y = Math.floor((1 - frame.score / maxScore) * canvas.height);
					ctx.lineTo(x, y);

					if (lastFill && lastFill !== fill) {
						ctx.lineTo(x, canvas.height);
						ctx.fill();
						ctx.beginPath();
						ctx.fillStyle = lastFill = fill;
						ctx.moveTo(x, canvas.height);
						ctx.lineTo(x, y);
					}

					if (!lastFill) ctx.fillStyle = lastFill = fill;
				}
				...
				ctx.beginPath();
				ctx.moveTo(0, Math.floor((1 - settings.graphBaseline / maxScore) * canvas.height));
				ctx.lineTo(canvas.width, Math.floor((1 - settings.graphBaseline / maxScore) * canvas.height));
				ctx.strokeStyle = '#888';
				ctx.lineWidth = 1.5;
				ctx.stroke();
				...
				if (mouse !== undefined) {
					ctx.beginPath();
					ctx.moveTo(Math.floor(mouse[0] * canvas.width), 0);
					ctx.lineTo(Math.floor(mouse[0] * canvas.width), canvas.height);
					ctx.stroke();
				}
			})();
