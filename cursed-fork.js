...
(async () => {
	...
	const aux = (() => {
        ...
        */
        ...
        /**
        * Function to check multiboxing support and handle multiboxing with one account
        * Uses BroadcastChannel API to communicate between tabs.
        * @param {object} aux - The auxiliary object containing necessary properties.
        */
        aux.multiboxAccSupport = () => {
            ...
            // Create a BroadcastChannel instance with a unique name (sigmod-sigfix)
            const channel = new BroadcastChannel("sigmod-sigfix");

            // Initialize auxiliary properties
            aux.isMultiboxing = false;
            aux.latestToken = "";

            // Send a message to check for multiboxing
            channel.postMessage({ type: "multibox" });

            /**
            * Function to send user token through the channel
            */
            function sendToken() {
                if (!aux.userData) return;
                channel.postMessage({ type: "token", content: aux.userData.token });
            }

            // Event listener for messages from other tabs
            channel.addEventListener("message", (e) => {
                console.log(e.data);
                switch (e.data.type) {
                    case 'multibox':
                        // Detected multiboxing, set flag and request token
                        aux.isMultiboxing = true;
                        console.log("Multiboxing detected");
                        channel.postMessage({ type: "get-token" });
                        break;
                    case 'get-token':
                        // Request to get token from other tabs
                        sendToken();
                        break;
                    case 'token': {
                        // Received token from another tab
                        const token = e.data.content;
                        if (aux.isMultiboxing) {
                            aux.latestToken = token;
                        }
                        break;
                    }
                    case 'tabClosed':
                        // Tab closed event, reset multiboxing flag
                        aux.isMultiboxing = false;
                        break;
                }
            });
            ...
            window.addEventListener('beforeunload', function() {
                // Notify other tabs about tab closure
                channel.postMessage({ type: 'tabClosed' });
            });
        }

        /**
         * The initialization of multiboxing support is delayed by 7 seconds to ensure
         * that the auth request is made twice. The second request uses the latest token.
        */
        setTimeout(aux.multiboxAccSupport, 7000);
	...
	const input = (() => {
		...
		function playData(spectating) {
			...
			if (aux.userData) {
				...
				token = aux.isMultiboxing ? aux.latestToken : aux.userData.token;
			}
